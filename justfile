...
launch_aux_services:
  docker compose -f ./aux/docker-compose.yml up \
  -d --wait --quiet-pull

# Stop auxiliary services, such as SQL database and SMTP server
halt_aux_services:
  docker compose -f ./aux/docker-compose.yml down --remove-orphans

test: launch_aux_services && halt_aux_services
  -deno test --unstable --allow-net --allow-env --allow-read
...
@debug:
  ...
  @deno run --inspect-brk --unstable --allow-net --allow-env --allow-read /main.ts